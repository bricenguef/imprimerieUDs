...
public class ImprimerieStateless implements GestionContactRemote{
	   ...
	   public Personnel rechercherPersonnel(String id) {
	      return em.find(Personnel.class, Integer.parseInt(id));
	   }

	   public List<Personnel> listerGroupe() {
	      return em.createQuery("SELECT p FROM Personnel p ORDER BY p.idpersonne").getResultList();
	   }
	...
	public List<Tache> listertache() {
		// TODO Auto-generated method stub
		return em.createQuery("SELECT p FROM tache p ORDER BY p.idtache").getResultList();
	}
